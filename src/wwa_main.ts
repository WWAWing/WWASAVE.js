...
module wwa_main {
    import KeyCode = wwa_input.KeyCode;
    import KeyState = wwa_input.KeyState;
    ...
    export class WWA {
        ...
        // WWASAVE用変数
        private _wwasave_savedata : { [key: string]: any; } = {};
        ...
        // wwa save用関数
        private _localstorage_init(){
			// wwasave
            // localstrage読み込みと初期設定
            this._wwasave_savedata.user_id = window.localStorage.getItem('wwasave_id');
            this._wwasave_savedata.user_token = window.localStorage.getItem('wwasave_token');
			this._wwasave_savedata.save_id = Number( window.localStorage.getItem('wwasave_savedata_id') );
			if(this._wwasave_savedata.user_id && this._wwasave_savedata.user_token && this._wwasave_savedata.save_id){
				if(this._wwasave_savedata.save_id === -1){
					// new game
					this._create_new_savedata();
				}
				else{
					// load game
					this.load_savedata();
        ...
        // セーブの新規作成
        private _create_new_savedata(){
			this._wwasave_savedata.id = -1;
			this.save_savedata();
			this._show_update();
		}
		// データの更新
		private _data_update(){
			this._wwasave_savedata.hp = this._player.getStatus().energy;
			this._wwasave_savedata.at = this._player.getStatus().strength;
			this._wwasave_savedata.df = this._player.getStatus().defence;
			this._wwasave_savedata.money = this._player.getStatus().gold;
			this._wwasave_savedata.player_x = this._player.getPosition().getPartsCoord().x;
			this._wwasave_savedata.player_y = this._player.getPosition().getPartsCoord().y;
			// パスワードセーブを入れる
			this._wwasave_savedata.long_password = this._quickSave(true);
			this._wwasave_savedata.comment = $("#save_comment").val();
		}
		// 表示の更新
		private _show_update(){
			$("#user_id").html(this._wwasave_savedata.user_id);
			$("#save_id").html(this._wwasave_savedata.id);
			$("#status_hp").html(this._wwasave_savedata.hp);
			$("#status_at").html(this._wwasave_savedata.at);
			$("#status_df").html(this._wwasave_savedata.df);
			$("#status_money").html(this._wwasave_savedata.money);
			$("#status_time").html(this._wwasave_savedata.save_time);
			$("#status_comment").html(this._wwasave_savedata.comment);
		}
		// LOAD
		public load_savedata(){
			$.ajax({
				url: '../server/index.php/save_get/' + this._wwasave_savedata.save_id + "/",
				type: 'GET',
				headers: {
					'id'   : this._wwasave_savedata.user_id,
					'token': this._wwasave_savedata.user_token,
				},
			}).done((data)=>{
				console.log("load_save_data : "+data);
				this._wwasave_savedata.id = data["id"];
				this._wwasave_savedata.user_id = data["user_id"];
				this._player.setEnergy( data["hp"]);
				this._player.setStrength( data["at"] );
				this._player.setDefence( data["df"] );
				this._player.setGold( data["money"] );
				this._player.updateStatusValueBox();
				this._player.jumpTo(new Position(this, data["player_x"], data["player_y"], 0, 0));
				this._wwasave_savedata.long_password　= data["long_password"];
				this._quickLoad( false, this._wwasave_savedata.long_password, true );
				// TODO: QuickLoadを試みる
				this._wwasave_savedata.save_time = data["save_time"];
				$("#save_comment").html(data["comment"]);
		...
		// SAVE
		public save_savedata(){
			// ステータスの更新
			this._data_update();
			let submit_data = {
				'token': this._wwasave_savedata.user_token,
				'user_id': this._wwasave_savedata.user_id,
				'hp': this._wwasave_savedata.hp,
				'at': this._wwasave_savedata.at,
				'df': this._wwasave_savedata.df,
				'money':this._wwasave_savedata.money,
				'player_x': this._wwasave_savedata.player_x,
				'player_y': this._wwasave_savedata.player_y,
				'long_password': this._wwasave_savedata.long_password,
				'comment': this._wwasave_savedata.comment
			};
			if(this._wwasave_savedata.id !== -1){
				submit_data['id'] = this._wwasave_savedata.id;
			}
			// SAVEを行う
			$.ajax({
				...
				this._wwasave_savedata.id = data["save_id"];
				this._show_update();
			}).fail((xhr)=>{
				// 失敗
				$("#password_save_info").html("");
